...
\begin{document}
    ...
    \begin{abstract}
        En este marco teórico se investigan la información de entrada para empezar a emplear la tarjeta microcontrolador ESP32 así como ejemplos de código en C++ Arduino IDE y MicroPython.
    \end{abstract}
...
Para utilizar las entradas discretas, de igual forma, se utiliza \say{pinMode(ledPin, INPUT);} para emplearla igual que cualquier programa Arduino.
...
Para la creación del programa se recomienda utilizar Thonny IDE. Entonces se deberá de seleccionar en el IDE el intérprete y el puerto del dispositivo. Ahí mismo en la pestaña de Intérprete, se selecciona en la parte de Firmware para flashear el firmware que se descargó. El nombre del archivo debe ser \say{main.py} ya que se carga \say{boot.py} de primero y luego \say{main.py}.
